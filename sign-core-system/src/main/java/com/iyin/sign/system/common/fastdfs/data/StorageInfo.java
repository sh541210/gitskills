...
import com.iyin.sign.system.common.fastdfs.command.AbstractCmd;
...
public class StorageInfo {
	...
	public StorageInfo(byte[] data,int offset) {
		log.info("StorageInfo: {}/{}", data.length, offset);
		status = data[offset];
		offset += 1;
		...
		this.connectionAllocCount = AbstractCmd.buff2int(data, offset);
		offset += AbstractCmd.FDFS_PROTO_INT_LEN_SIZE;
		this.connectionCurrentCount = AbstractCmd.buff2int(data, offset);
		offset += AbstractCmd.FDFS_PROTO_INT_LEN_SIZE;
		this.connectionMaxCount = AbstractCmd.buff2int(data, offset);
		offset += AbstractCmd.FDFS_PROTO_INT_LEN_SIZE;
