...
public class PdfSignatureAppearance {
    ...
    public void setVisibleSignature(Rectangle pageRect, int page, String fieldName) {
        ...
        if (page < 1) {
            page = 1;
        } else if (page > writer.reader.getNumberOfPages()) {
            page = writer.reader.getNumberOfPages();
        }
    ...
    public void setVisibleSignature(List<String> fieldNames) {
    	this.fieldNames=fieldNames;
		for(String fieldName:fieldNames) {
			setVisibleSignature(fieldName);
		}
    }
    ...
    public enum RenderingMode {
        ...
        /**
         * The rendering mode is multiple image.
         */
        MULTIPLE_GRAPHIC
    }
    ...
    */
    ...
    private List<PdfTemplate> signatureGraphices=null;
    
	public List<PdfTemplate> getSignatureGraphices() {
		return signatureGraphices;
	}

	public void setSignatureGraphices(List<PdfTemplate> signatureGraphices) {
		this.signatureGraphices = signatureGraphices;
	}
    ...
    */
    ...
    public PdfTemplate getAppearance() throws DocumentException {
        ...
        if (app[2] == null) {
            ...
            case GRAPHIC:
                ...
                ct2.go();
                break;
            case MULTIPLE_GRAPHIC:
            	 if (CollectionUtils.isEmpty(signatureGraphices)) {
                     throw new IllegalStateException(MessageLocalization.getComposedMessage("a.signature.images.should.be.present.when.rendering.mode.is.multiple.graphic"));
                 }
            	break;
            default:
            }
    ...
    private void redoSign(AcroFields af,PdfIndirectReference refSig) {
    	ArrayList<String> names = af.getSignatureNames();
        if(!names.isEmpty()) {
        	for(int i=0;i<names.size();i++) {
        		boolean fieldExists = af.doesSignatureFieldExist(names.get(i));
        		 if (fieldExists) {
        	            PdfDictionary widget = af.getFieldItem(names.get(i)).getWidget(0);
        	            writer.markUsed(widget);
//        	            PdfDictionary fieldLock = widget.getAsDict(PdfName.LOCK);
...
//        	            widget.put(PdfName.AP, ap);
    ...
    private PdfDictionary buildImageLayer(PdfDictionary widget,PdfIndirectReference refSig,PdfObject pdfObject,int page) {
    	 writer.markUsed(widget);
    	 PdfDictionary fieldLock = widget.getAsDict(PdfName.LOCK);
	...
	public void preClose(HashMap<PdfName, Integer> exclusionSizes) throws IOException, DocumentException {
        ...
        if (fieldExists) {
        	if(renderingMode == RenderingMode.MULTIPLE_GRAPHIC) {
        		for(int i=0;i<getFieldNames().size();i++) {
        			int page=af.getFieldItem(getFieldNames().get(i)).getPage(0);
        			PdfDictionary widget = af.getFieldItem(getFieldNames().get(i)).getWidget(0);
        			PdfDictionary lock=buildImageLayer(widget, refSig, getSignatureGraphices().get(i).getIndirectReference(),page);
					if (null != lock) {
						fieldLock = lock;
					}
        		}
        		...
        		fieldLock=buildImageLayer(widget, refSig, getAppearance().getIndirectReference(),getPage());
        	}
        }
        else {
